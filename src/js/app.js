...
const movieID = Math.random().toFixed(1) > 0.5
  ? marvelMovieID
  : dcmovieID;
      ...
      resultTitle.map(i => $event(i, 'click', () => {
        ...
        xhr.onload = () => {
          ...
          if (xhr.status === 200) {
            ...
            let budgetDivide = output.budget > setBillion
              ? setBillion
              : setMillion;
            // check revenue is over Billion or not
            let revenueDivide = output.revenue > setBillion
              ? setBillion
              : setMillion;
            // check budget unit
            let budgetUnit = output.budget > setBillion
              ? `Billion(s)`
              : `Million(s)`
            // check revenue unit
            let revenueUnit = output.revenue > setBillion
              ? `Billion(s)`
              : `Million(s)`;
